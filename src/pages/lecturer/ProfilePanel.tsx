  ...
  return (
    <div className="px-4 py-4">
      ...
      <div className="mb-5 flex flex-col items-center text-center">
        ...
        <h1 className="text-lg font-semibold text-zinc-900 dark:text-white">{lecturer.fullName}</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">{lecturer.institutionalId} · Lecturer</p>
      </div>

      <div className="space-y-2">
        <ProfileRow label="Department" value={department?.name ?? '—'} />
        <ProfileRow icon={Mail} label="Email" value={lecturer.email} />
        <ProfileRow icon={Phone} label="Phone" value={lecturer.phoneMasked} />
        <ProfileRow icon={ShieldCheck} label="Face ID" value={lecturer.faceIdEnrolled ? 'Enrolled' : 'Not enrolled'} tone={lecturer.faceIdEnrolled ? 'good' : 'warn'} />
        {lecturer.deviceModel && <ProfileRow icon={Smartphone} label="Device" value={lecturer.deviceModel} />}
      </div>
    </div>
  );
}
